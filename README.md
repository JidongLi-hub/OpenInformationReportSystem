...
## 备选方案：DeepSeek 云端 API 模式 (无显卡环境)

当本地服务器显卡资源紧张（如显存被占满）时，可以使用此模式。系统将调用 DeepSeek-V3 云端大模型进行推理，同时保留本地 RAG（检索增强生成）能力。若本地向量服务也无法启动，系统将自动降级为“模拟情报+真实模型”模式，确保演示流程畅通。

### 1. 配置密钥 (必需)

在项目根目录下新建 `.env` 文件（**注意：此文件已在 .gitignore 中，切勿提交到仓库**），填入你的 API Key：
...
使用 `server_deepseek_api.py` 替代原有的启动方式。此脚本集成了后端业务与静态托管，且不需要启动 `start_vllm.py`。
...
* **自动鉴权**: 启动时会自动读取 `.env` 中的密钥，若未配置会报错提示。
* **智能降级**:
    * **优先**: 连接本地 `database.py` 进行真实向量检索 + 调用 DeepSeek 生成报告。
    * **降级**: 若本地数据库/Embedding服务未启动，自动使用**模拟情报数据** + 调用 DeepSeek 生成报告。
* **端口避让**: 默认从 `29001` 开始寻找空闲端口，避免与本地版（28001/28888）发生冲突，支持两套系统同时运行。

### 4. 访问方式

查看终端输出的动态端口（例如 29001），使用 SSH 隧道映射后访问：

```bash
ssh -L 29001:localhost:29001 用户名@服务器IP
```